...
function askSingle(question, options, callback, _output) {
	...
	if (options.overrideDefaults && options.overrideDefaults[options.key]) {
		options.default = options.overrideDefaults[options.key];
	}

	// Prompt depends
	switch (typeof options.depends) {
		case 'boolean':
			if (!options.depends) return callback(null, options.default);
			break;
		case 'function':
			if (!options.depends(_output)) return callback(null, options.default);
			break;
	}
